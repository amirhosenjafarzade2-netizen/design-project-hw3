# -------------------------------------------------
# BNA Reservoir Visualizer – FULLY FIXED & FINAL
# -------------------------------------------------
...
# DATAFRAME HELPERS
# ========================================
def contours_to_full_df(contours):
    """All points → for contour/heatmap interpolation"""
    xs, ys, zs = [], [], []
    for c in contours:
...
def contours_to_hist_df(contours):
    """One point per contour (centroid) → for histogram"""
    data = []
    for c in contours:
        pts = c["points"]
        center_x = pts[:, 0].mean()
        center_y = pts[:, 1].mean()
        data.append({"x": center_x, "y": center_y, "z": c["z"]})
...
def plot_histogram(ax, df, title, xlabel, ylabel, unit, bins):
    ...
    name = title.lower()
    color = ("#1f77b4" if "poro" in name else
             "#d62728" if "perm" in name else
             "#2ca02c" if "ntg" in name or "net" in name else
             "#ff7f0e" if "thick" in name else
             "#9467bd")
    ...
    ax.set_yticklabels([f"{int(v)}%" for v in np.linspace(0, 100, 6)])
...
for f in uploaded_files:
    try:
        ...
        all_contours[f.name] = parse_bna(content)
...
for fname in all_contours.keys():
    with st.sidebar.expander(fname, expanded=False):
        default_title = os.path.splitext(fname)[0].replace("_", " ").replace(".bna", "")
        ...
        levels = cmap = distribution = vario_type = bins = hist_ylabel = None
        ...
        elif map_type == "Property Heatmap":
        ...
        config[fname] = {
            ...
            "unit": unit, "levels": levels, "cmap": cmap,
            "distribution": distribution, "vario_type": vario_type,
            "bins": bins, "hist_ylabel": hist_ylabel
        }

# ========================================
# PLOTTING
# ========================================
plot_mode = st.radio("Plot Mode", ["Single File", "All Files (Batch)"], horizontal=True)

if plot_mode == "Single File":
    file = st.selectbox("Select File", options=list(all_contours.keys()))
    cfg = config[file]
    contours = all_contours[file]
    full_df = contours_to_full_df(contours)
    hist_df = contours_to_hist_df(contours)

    fig, ax = plt.subplots(figsize=(11, 9), dpi=160)
    mt = cfg["map_type"]

    if mt in ["Structure Map", "Thickness Map"]:
        plot_contour_map(ax, full_df, cfg["title"], cfg["xlabel"], cfg["ylabel"],
                         cfg["unit"], cfg["levels"], cfg["cmap"])
    elif mt == "Property Heatmap":
        plot_property_heatmap(ax, full_df, cfg["title"], cfg["xlabel"], cfg["ylabel"],
                              cfg["unit"], cfg["cmap"], cfg["distribution"], cfg["vario_type"])
    elif mt == "Histogram":
        plot_histogram(ax, hist_df, cfg["title"], cfg["xlabel"], cfg["hist_ylabel"],
                       cfg["unit"], cfg["bins"])
        ...
        if s_file and t_file:
            plot_overlay(ax, contours_to_full_df(all_contours[s_file]),
                        contours_to_full_df(all_contours[t_file]))
        else:
            st.error("Need both Structure and Thickness files for overlay.")
            st.stop()
    ...
    if st.checkbox("Export Interpolated Grid CSV"):
        xi = np.linspace(full_df["x"].min(), full_df["x"].max(), 500)
        yi = np.linspace(full_df["y"].min(), full_df["y"].max(), 500)
        Xi, Yi = np.meshgrid(xi, yi)
        Zi = griddata((full_df["x"], full_df["y"]), full_df["z"], (Xi, Yi), method="cubic")
        grid_df = pd.DataFrame({"X": Xi.ravel(), "Y": Yi.ravel(), "Z": Zi.ravel()})
        csv = grid_df.to_csv(index=False).encode()
